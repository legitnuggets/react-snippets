...
export const replaceSnippetPlaceholders = (snippetString: string) => {
    ...
    const propsPlaceholder = typescriptPropsStatePrefix === "type" ? Mappings.TypeProps : Mappings.InterfaceProps;
    const statePlaceholder = typescriptPropsStatePrefix === "type" ? Mappings.TypeState : Mappings.InterfaceState;
    ...
    return String(snippetString)
        ...
        .replace(new RegExp(Placeholders.Capitalize, "g"), "${1/(.*)/${1:/capitalize}/}")
...
export const revertSnippetPlaceholders = (snippetString: string) => {
    return String(snippetString)
        .replace(new RegExp(/\${1:\${TM_FILENAME_BASE}}/, "g"), Placeholders.FileName)
        ...
        .replace(new RegExp(/\${1\/(.*)\/${1:\/capitalize}\/}/, "g"), Placeholders.Capitalize);
};
